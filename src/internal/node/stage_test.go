...
// TestTouchedReadsTheDiffAndNotTheTree. A repository that has always held
// `migrations/` would otherwise stop every task forever; the question is what
// *this* stage did.
func TestTouchedReadsTheDiffAndNotTheTree(t *testing.T) {
	repo := t.TempDir()
	...
	// The migration is already there before this stage starts.
	write("migrations/001_initial.sql", "create table t;")
	...
	// The stage changes only ordinary code.
	write("app.go", "package app\n\n// more\n")
	run("add", "-A")
	run("commit", "-qm", "ordinary work")
	ordinary := strings.TrimSpace(mustGit(t, repo, "rev-parse", "HEAD"))

	patterns := []string{"migrations/", ".env"}
	if hit := Touched(context.Background(), repo, base, ordinary, patterns); len(hit) != 0 {
		t.Errorf("an ordinary delivery matched the guard: %v", hit)
	}

	// And now it touches one.
	write("migrations/002_drop.sql", "drop table t;")
	run("add", "-A")
	run("commit", "-qm", "a migration")
	risky := strings.TrimSpace(mustGit(t, repo, "rev-parse", "HEAD"))

	hit := Touched(context.Background(), repo, ordinary, risky, patterns)
	if len(hit) != 1 || !strings.Contains(hit[0], "002_drop.sql") {
		t.Errorf("the guarded path was not reported: %v", hit)
	}
}

// TestAnUnreadableDiffMatchesEverything. This is the one place in the node where
// the cautious answer is the noisy one: failing open would let a migration land
// unattended because git hiccupped, and a guard that goes silent when it breaks
// is worse than no guard.
func TestAnUnreadableDiffMatchesEverything(t *testing.T) {
	patterns := []string{"migrations/", ".env"}

	hit := Touched(context.Background(), t.TempDir(), "nope", "alsonope", patterns)
	if len(hit) != len(patterns) {
		t.Errorf("a diff that could not be read reported %v, want every pattern", hit)
	}
}

// TestNoPatternsMatchNothing, so a stage that declares no guard pays for no git
// call at all.
func TestNoPatternsMatchNothing(t *testing.T) {
	if hit := Touched(context.Background(), t.TempDir(), "a", "b", nil); hit != nil {
		t.Errorf("a stage with no guard reported %v", hit)
	}
}

// mustGit runs a git command and returns its output, failing the test otherwise.
