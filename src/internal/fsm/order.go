...
// to "do nothing" is a task that stops with nobody knowing (INV-core-8).
...
const (
	...
	// was asked something and has not answered (ADR-0013).
...
// prevent (INV-core-1, RFC-0002). The lead is an agent so a person can talk to
...
type Order struct {
	...
	// (ADR-0040). Empty means the stage is mechanical: Luna runs it itself, with
	// no agent at all.
	...
	// than a line in a brief (INV-core-7, RFC-0002).
	...
	// description of it (INV-core-6). Empty on the first stage of a task, where
	...
	// that is not told what it owes will not deliver it (INV-core-3).
...
// no process (ADR-0024). That is what lets the order be tested without
...
// "compounds at every hop" (RFC-0002). This one is branched from the previous
...
// agent is wired to it; JSON is what the lead reads once one is (RFC-0002). The
